...
func (d *DB) Load(ctx context.Context, ecosystems []model.Ecosystem) (*Index, error) {
	...
	for _, e := range ecosystems {
		...
		count, err := loadArchive(ctx, archive, e, idx)
		if err != nil {
...
func loadArchive(ctx context.Context, path string, e model.Ecosystem, idx *Index) (int, error) {
	...
	for i, entry := range r.File {
		...
		if err != nil {
			...
			// the remaining thousands are still worth having.
			continue
		}
		if !ok {
			continue
